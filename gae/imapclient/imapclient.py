...
if 'XLIST' not in imaplib2.Commands:
    imaplib2.Commands['XLIST'] = imaplib2.Commands['LIST']

# ...and IDLE
if 'IDLE' not in imaplib2.Commands:
    imaplib2.Commands['IDLE'] = imaplib2.Commands['APPEND']
...
class IMAPClient(object):
    ...
    *normalise_times* is True (times are normalised to the local
    ...
    Error = imaplib2.IMAP4.error
    AbortError = imaplib2.IMAP4.abort
    ReadOnlyError = imaplib2.IMAP4.readonly
    ...
    def _create_IMAP4(self):
        ...
        if self.stream:
            return imaplib2.IMAP4_stream(self.host)
        ImapClass = self.ssl and imaplib2.IMAP4_SSL or imaplib2.IMAP4
    ...
    def login(self, username, password):
        ...
        server response.
        """
        return self._command_and_check('login', username, password,
                                       unpack=True)
    ...
    def oauth2_login(self, user, access_token):
        ...
        """
        auth_string = lambda x: 'user=%s\1auth=Bearer %s\1\1' % (
        user, access_token)
    ...
    def get_folder_delimiter(self):
        ...
        warnings.warn(DeprecationWarning(
            'get_folder_delimiter is going away. Use namespace() instead.'))
    ...
    def select_folder(self, folder, readonly=False):
        ...
        """
        # manu: disabled flags processing, we don't need them anyway
        self._command_and_check('select', self._normalise_folder(folder),
                                readonly)
    ...
    def _process_select_response(self, resp):
        ...
        for key, value in resp:
            ...
            'EXISTS', 'RECENT', 'UIDNEXT', 'UIDVALIDITY', 'HIGHESTMODSEQ'):
                value = int(value[0])
    ...
    def idle_check(self, timeout=None):
        ...
        """
        # In py2, imaplib2 has sslobj (for SSL connections), and sock for non-SSL.
            ...
            if rs:
                while True:
                    try:
                        ...
                        # An imaplib2.IMAP4.abort with "EOF" is raised
                        # under Python 3
                        err = sys.exc_info()[1]
    ...
    def folder_status(self, folder, what=None):
        ...
        *what* should be a sequence of status items to query. This
        ...
        data = self._command_and_check('status',
                                       self._normalise_folder(folder), what_,
                                       unpack=True)
    ...
    def create_folder(self, folder):
        ...
        """
        return self._command_and_check('create',
                                       self._normalise_folder(folder),
                                       unpack=True)
    ...
    def delete_folder(self, folder):
        ...
        """
        return self._command_and_check('delete',
                                       self._normalise_folder(folder),
                                       unpack=True)
    ...
    def folder_exists(self, folder):
        ...
        """
        data = self._command_and_check('list', '""',
                                       self._normalise_folder(folder))
    ...
    def add_gmail_labels(self, messages, labels):
        ...
        *get_gmail_labels*).
        ...
        attribute (eg. Gmail).
        """
        labels = [self._normalise_folder(l) for l in labels]
    ...
    def fetch(self, messages, data, modifiers=None):
        ...
        *modifiers* are required for some extensions to the IMAP
        ...
        return parse_fetch_response(from_bytes(data), self.normalise_times,
                                    self.use_uid)
    ...
    def getacl(self, folder):
        ...
        """
        data = self._command_and_check('getacl',
                                       self._normalise_folder(folder))
...
def seq_to_parenstr_upper(items):
    return _join_and_paren(
        item.upper() for item in _normalise_text_list(items))
